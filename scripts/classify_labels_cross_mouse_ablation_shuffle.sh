...
#SBATCH --output=logs/slurm-zz-cross-shuf-%j.out
#SBATCH --error=logs/slurm-zz-cross-shuf-%j.err
...
#   - N_SHUFFLES IDs are randomly sampled from the INTERSECTION of available
...
mkdir -p "${OUT_DIR}"
echo "Starting cross-mouse shuffle classification ..."
"${CMD[@]}"
EXIT_CODE=$?
