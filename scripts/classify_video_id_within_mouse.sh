...
#SBATCH --job-name=zz-within-video-id
...
#SBATCH --output=logs/slurm-zz-within-video-id-%j.out
#SBATCH --error=logs/slurm-zz-within-video-id-%j.err

set -euo pipefail

PROJECT_DIR="/u/mdmc/anaflom/projects_mdmc/ZigZagSensorium"
SCRIPT="${PROJECT_DIR}/scripts/classify_video_id_within_mouse.py"
...
OUTPUT_BASE="${OUTPUT_BASE:-${PROJECT_DIR}/results/within_mouse_video_id_decoding/p${P_ACTIVE}-${OUTPUT_SUFFIX}}"
RUN_TS="$(date +%Y%m%d_%H%M%S)"
RUN_TAG="p${P_ACTIVE}_method-${VECTORIZATION_METHOD}_minrep-${MIN_ID_REPETITIONS}_cv-${CV_SCHEME}_${RUN_TS}"
...
echo "CV_SCHEME: ${CV_SCHEME} (n_splits=${CV_N_SPLITS})"
echo "============================================"
