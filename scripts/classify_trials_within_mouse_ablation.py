    ...
    with open(log_path, "w", encoding="utf-8") as log_fp, redirect_stdout(log_fp), redirect_stderr(log_fp):
        ...
        for mouse_name in selected_mice:
            ...
            try:
                ...
                cm = confusion_matrix(y_int, best_pred, labels=np.arange(len(class_labels)))
        ...
        # Figure 3: Confusion matrices for best model per mouse.
        n = len(mice_order)
        n_cols = min(3, n)
        ...
        for ax, mouse_name in zip(axes_flat, mice_order):
            ...
            best_model = confusion_payload[mouse_name]["model"]
            ...
            ax.set_title(f"{mouse_name}\n{model_titles[best_model]}", fontsize=8)

        for ax in axes_flat[len(mice_order):]:
            ax.set_visible(False)

        fig.suptitle("Normalized confusion matrices (best model per mouse)", fontsize=12)
        fig.tight_layout()
        fig3 = figures_dir / "03_best_model_confusion_matrices.png"
